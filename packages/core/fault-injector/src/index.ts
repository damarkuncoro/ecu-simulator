...
export interface FaultInjectionConfig {
  enabled: boolean;
  globalProbability: number; // 0.0 to 1.0
...
const DEFAULT_FAULTS: Record<string, FaultCondition> = {
...
export class FaultInjector {
  ...
  private faults = new Map<string, FaultCondition>();
  ...
  getActiveFaults(): FaultCondition[] {
    return Array.from(this.activeFaults)
      ...
      .filter(Boolean) as FaultCondition[];
  }
  ...
  triggerRandomFault(): FaultCondition | null {
    if (!this.shouldTriggerRandomFault()) return null;

    const availableFaults = this.getAllFaults().filter(
      (fault) =>
        !this.activeFaults.has(fault.id) && Math.random() < fault.probability,
    );

    if (availableFaults.length === 0) return null;

    const randomFault =
      availableFaults[Math.floor(Math.random() * availableFaults.length)];
    if (!randomFault) return null;
  ...
  getStats(): {
    ...
    const totalTriggered = Array.from(this.faults.values()).reduce(
      (sum, fault) => sum + fault.triggerCount,
      0,
    );
  ...
  private activateFault(fault: FaultCondition): boolean {
    if (this.activeFaults.has(fault.id)) return false;
  ...
  private deactivateFault(fault: FaultCondition): void {
